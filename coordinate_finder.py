    ...
    for i in range(0, l):
        for j in range(0, l - i - 1):
            if (sub_li[j][0] > sub_li[j + 1][0]):
                ...
                sub_li[j + 1] = tempo
    if(l==4):
        if (sub_li[0][1] > sub_li[1][1]):
            tempo = sub_li[0]
            sub_li[0] = sub_li[1]
            sub_li[1] = tempo
        if (sub_li[2][1] > sub_li[3][1]):
            tempo = sub_li[2]
            sub_li[2] = sub_li[3]
            sub_li[3] = tempo
    return sub_li
...
def trisect_points(point):
    ...
    distance = ((point[2][0] - point[1][0])**2 + (point[2][1] - point[1][1])**2)**0.5

    # Calculate the trisect points
    trisect_distance = distance / 3
    theta = math.atan2(point[2][1] - point[1][1], point[2][0] - point[1][0])
    trisect1_x = point[1][0] + trisect_distance * math.cos(theta)
    trisect1_y = point[1][1] + trisect_distance * math.sin(theta)
    trisect2_x = point[1][0] + 2 * trisect_distance * math.cos(theta)
    trisect2_y = point[1][1] + 2 * trisect_distance * math.sin(theta)
